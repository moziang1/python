...
def changanqiehuan():
    ...
    datangguanfu1.place_forget()    #关闭第二背景传送按钮
    theLabel2.pack_forget()        #关闭第二背景图
    datanguanfuyinyue()           #开启大唐官府门口音乐
    datangguanfu2.place(x=330,y=200)             #开启大唐官府的按钮（接任务）点击npc
...
def changan():        #row要用来后面传参用别删
    ...
    #定义一个按钮，在这里部署下一步，跳转到大唐官府背景，然后说话，然后跳转到建业城#打怪，然后结束

    #这个界面就是清除第一章的东西，然后铺上第二章背景
    ...
    #部署传送按钮
    datangguanfu1.place(x=510,y=150)
...
#部署一个按钮在第2章背景图处，用于传送第2背景到第3背景地图
datangguanfu1 = tk.Button(win, text='对话 传送', bg='black', fg='white',command=changanqiehuan)


#部署一个按钮在第3章背景图外，（进入程咬金官府）
datangguanfu2 = tk.Button(win, text='对话', bg='black', fg='white',command=dtgffy)

#部署一个按钮在第3章背景图内，（离开程咬金官府）
datangguanfu3 = tk.Button(win, text='接受', bg='black', fg='white',command=jianketp)

#部署一个按钮在第2章背景图处，（传输建邺城）
